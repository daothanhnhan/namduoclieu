...
class action_menu extends library{
    ...
    /*-----------------------------*/
    public function showMenu_byMultiLevel_mainMenuMia($list_menu,$valParent_menu = 0,$lang, $level = 0){
        $cate_child = array();
        foreach ($list_menu as $key => $item)
        {
            // Nếu là chuyên mục con thì hiển thị
                ...
                if($level == 0 && $row[$this->nameColTitle_menuLanguage] == "Trang chủ")
                  echo '<li  class="has-sub">'; 
                    ...
                    if($level == 0 && $row[$this->nameColTitle_menuLanguage] != "Trang chủ")
                        ...
                        echo '<li  class="has-sub">'; 
                if ($url == '/sale') {$style = 'style="color:white;"';} else {$style = '';}
                 echo '<a href="'.$url.'" class="slide-section" '.$style.' >'.$row[$this->nameColTitle_menuLanguage].'</a>';

                 if ($level == 0 && $row1['menu_type_id'] == 3) {
                    $img = $this->getProductCatImg($row1['productcat_id']);
                    $this->danhMucCapHai($list_menu,$item[$this->nameColID_menu],$lang, $level+1, $img);
                 }
    ...
    public function danhMucCapHai ($list_menu, $valParent_menu = 0, $lang, $level, $img) {
        $cate_child = array();
        foreach ($list_menu as $key => $item)
        {
            // Nếu là chuyên mục con thì hiển thị
        ...
        echo '<div class="sub-menu">
                        <div class="row">';
                            $d = 0;
                            $count = count($cate_child);
                            foreach ($cate_child as $key => $item) {
                                $d++;
                                $row = $this->getMenuLanguageDetail_byId($item[$this->nameColID_menu],$lang);
                                $col = ($d%3==0) ? '2' : '3';
                            echo '<div class="col-xs-'.$col.'"><h3>'.$row[$this->nameColTitle_menuLanguage].'</h3>';
                            $this->danhMucCapBa($list_menu,$item[$this->nameColID_menu],$lang, $level+1);
                            echo '</div>';
                            }
        if ($count == 2) { echo '<div class="col-xs-2"></div>'; }
        echo                '<div class="col-xs-4 banner">
                                <a href="">
                                    <img src="/images/'.$img.'"  alt="" class="img-responsive">
                                </a>
                            </div>
                        </div>
                    </div>';
    ...
    public function danhMucCapBa ($list_menu, $valParent_menu = 0, $lang, $level) {
        $cate_child = array();
        foreach ($list_menu as $key => $item)
        {
            // Nếu là chuyên mục con thì hiển thị
    ...
    public function getProductCatImg ($id) {
        global $conn_vn;
        $sql = "SELECT * FROM productcat WHERE productcat_id = $id";
        $result = mysqli_query($conn_vn, $sql);
        $row = mysqli_fetch_assoc($result);
        return $row['productcat_img'];
    }
}   
?>