...
public function getListNewsHot_hasLimit($limit){
    ...
    $limit = "limit ".$limit;
    //$sql = "SELECT * from $table $where $limit";    
    $sql = "SELECT * from $table where news_hot = 1 order by news_id desc $limit";    
...
/*------ lấy thông tin các tin tức view - @tmp -----*/
public function getListNewsView_hasLimit($limit){
    global $conn_vn;
    $table = $this->nameTable_news;
    //$where = "where $this->nameColHot_news != 0";
    $limit = "limit ".$limit;
    //$sql = "SELECT * from $table $where $limit";    
    $sql = "SELECT * from $table order by news_views desc $limit";    
...
public function addViewNews ($news_id) {
    global $conn_vn;
    $sql = "SELECT * FROM news WHERE news_id = $news_id";
    $result = mysqli_query($conn_vn, $sql);
    $row = mysqli_fetch_assoc($result);//var_dump($row);
    $view = (int)$row['news_views'] + 1;

    $sql = "UPDATE news SET news_views = $view WHERE news_id = $news_id";
    $result = mysqli_query($conn_vn, $sql);
}
